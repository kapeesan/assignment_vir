...
public class DocumentController {
	...
	@RequestMapping(value ="/document/{id}/card",method = RequestMethod.GET)
	public ResponseEntity<List<Card>> fetchc(@PathVariable Integer id){
		if(id<=0) {
			return ResponseEntity.badRequest().build();
		}
		else {
			Document document=documentService.fetch(id);
			List<Card> c=document.getCards();
			for(Card card:c) {
